...
fn discover_routing_db() -> Result<PathBuf, String> {
    ...
    candidates.sort();
    candidates.dedup();
    candidates.into_iter().find(|p| is_routing_db(p)).ok_or_else(|| "V2rayU routing database was not found".to_string())
}

fn decode_hex(value: &str) -> String {
    let bytes: Vec<u8> = value.as_bytes().chunks(2).filter_map(|pair| {
...
fn restart_v2rayu_app() -> Result<(), String> {
    let _ = Command::new("osascript").args(["-e", "tell application \"V2rayU\" to quit"]).status();
    std::thread::sleep(std::time::Duration::from_millis(500));
    let status = Command::new("open").args(["-a", "V2rayU"]).status().map_err(|e| e.to_string())?;
    if status.success() { Ok(()) } else { Err("failed to relaunch V2rayU".to_string()) }
}
...
pub fn list_routing_profiles() -> Result<Vec<RoutingProfile>, String> {
    ...
    for row in raw.lines() {
        ...
        profiles.push(RoutingProfile {
            uuid: uuid.clone(), name: decode_hex(cols[1]), remark: decode_hex(cols[2]),
            domain_strategy: decode_hex(cols[3]), domain_matcher: decode_hex(cols[4]),
            block: split_lines(decode_hex(cols[5])), proxy: split_lines(decode_hex(cols[6])), direct: split_lines(decode_hex(cols[7])),
            sort: cols[8].parse().unwrap_or(0), active: uuid == active,
        });
    }
    Ok(profiles)
}
...
pub fn save_routing_profile(profile: RoutingProfile, activate: bool) -> Result<String, String> {
    ...
    let sql = format!("UPDATE routing SET name='{}',remark='{}',domainStrategy='{}',domainMatcher='{}',block='{}',proxy='{}',direct='{}' WHERE uuid='{}'; SELECT changes();",
        sql_quote(&profile.name), sql_quote(&profile.remark), sql_quote(&profile.domain_strategy), sql_quote(&profile.domain_matcher), block, proxy, direct, sql_quote(&profile.uuid));
    let changed = output("sqlite3", &[dbs, &sql])?;
    if changed.lines().last().map(str::trim) != Some("1") { return Err("routing profile was not updated".to_string()); }
    if activate || profile.active {
        defaults_write_string("runningRouting", &profile.uuid)?;
        restart_v2rayu_app()?;
    }
    Ok(format!("Saved {} · backup {}", if profile.remark.is_empty() { &profile.name } else { &profile.remark }, backup.display()))
}
...
pub fn create_routing_profile(name: String) -> Result<String, String> {
    ...
    let backup = backup_database(&db)?;
    let dbs = db.to_str().ok_or_else(|| "invalid routing database path".to_string())?;
    let uuid = format!("v2proxyshell-{}", SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_millis());
    let max_sort = output_lossy("sqlite3", &[dbs, "SELECT COALESCE(MAX(sort),0)+1 FROM routing;"]).trim().parse::<i64>().unwrap_or(1);
    let title = if name.trim().is_empty() { "New routing".to_string() } else { name.trim().to_string() };
    let sql = format!("INSERT INTO routing(uuid,name,remark,domainStrategy,domainMatcher,block,proxy,direct,sort) VALUES('{}','{}','{}','AsIs','hybrid','','','',{});",
        sql_quote(&uuid), sql_quote(&title), sql_quote(&title), max_sort);
...
pub fn delete_routing_profile(uuid: String) -> Result<String, String> {
    let active = defaults_read("runningRouting");
    if active == uuid { return Err("The active routing profile cannot be deleted. Activate another profile first.".to_string()); }
    let db = discover_routing_db()?;
    let backup = backup_database(&db)?;
    let dbs = db.to_str().ok_or_else(|| "invalid routing database path".to_string())?;
    let sql = format!("DELETE FROM routing WHERE uuid='{}'; SELECT changes();", sql_quote(&uuid));
    let changed = output("sqlite3", &[dbs, &sql])?;
    if changed.lines().last().map(str::trim) != Some("1") { return Err("routing profile was not deleted".to_string()); }
...
pub fn set_proxy_mode(mode: String) -> Result<String, String> {
    ...
    if !matches!(normalized.as_str(), "pac" | "manual" | "global" | "tun") { return Err(format!("unsupported proxy mode: {mode}")); }
...
pub fn control_v2rayu(action: String) -> Result<String, String> {
    match action.as_str() {
        "start" => { defaults_write_bool("v2rayTurnOn", true)?; restart_v2rayu_app()?; }
        "stop" => { defaults_write_bool("v2rayTurnOn", false)?; restart_v2rayu_app()?; }
...
fn parse_nettop_counters() -> HashMap<u32, (u64, u64)> {
    ...
    for line in raw.lines() {
        ...
        let Some((_, pid_text)) = fields[0].rsplit_once('.') else { continue };
        let (Ok(pid), Ok(down), Ok(up)) = (pid_text.parse::<u32>(), fields[1].parse::<u64>(), fields[2].parse::<u64>()) else { continue };
...
fn endpoint(value: &str) -> (String, u16) {
    ...
    if value.starts_with('[') {
        if let Some(i) = value.rfind("]:") { return (value[1..i].to_string(), value[i + 2..].parse().unwrap_or(0)); }
    }
    value.rsplit_once(':').map(|(h, p)| (h.to_string(), p.parse().unwrap_or(0))).unwrap_or((value.to_string(), 0))
}

fn local_host(host: &str) -> bool {
    if matches!(host, "localhost" | "127.0.0.1" | "::1") { return true; }
    let Ok(ip) = host.parse::<std::net::IpAddr>() else { return false };
    match ip {
        std::net::IpAddr::V4(v4) => { let o = v4.octets(); v4.is_private() || v4.is_loopback() || v4.is_link_local() || (o[0] == 100 && (64..=127).contains(&o[1])) }
...
fn aggregate_connections() -> HashMap<u32, ConnAgg> {
    ...
    for line in raw.lines().skip(1) {
        ...
        if (rh == "127.0.0.1" || rh == "::1") && (rp == DEFAULT_HTTP_PORT || rp == DEFAULT_SOCKS_PORT) { agg.proxy += 1; }
        else if (lh == "127.0.0.1" || lh == "::1") && (lp == DEFAULT_HTTP_PORT || lp == DEFAULT_SOCKS_PORT) { agg.xray += 1; }
        else if process.to_ascii_lowercase().contains("xray") || process.to_ascii_lowercase().contains("v2ray") { agg.xray += 1; }
        else if system_process(&process) { agg.system += 1; }
        else if local_host(&rh) { agg.local += 1; }
        else { agg.direct += 1; agg.direct_targets.insert(format!("{rh}:{rp}")); }
    }
    map
}

#[tauri::command]
pub fn get_traffic_snapshot(state: tauri::State<'_, MonitorState>) -> Result<Vec<TrafficRow>, String> {
    let now = Instant::now();
    ...
    for pid in pids {
        ...
        let (proxy, direct, local, system, xray, targets) = c.map(|x| (x.proxy, x.direct, x.local, x.system, x.xray, x.direct_targets.iter().cloned().collect::<Vec<_>>())).unwrap_or((0,0,0,0,0,vec![]));
        let mode = if proxy > 0 && direct > 0 { "MIXED" } else if direct > 0 { "DIRECT" } else if proxy > 0 { "PROXY" } else if system > 0 { "SYSTEM" } else if xray > 0 || process.to_ascii_lowercase().contains("xray") || process.to_ascii_lowercase().contains("v2ray") { "XRAY" } else { "LOCAL" };
